...
function Draggable(props) {
    ...
    return (
        <div onDragStart={handleDragStart} onDragEnd={handleDragEnd} className={className} draggable>{children}</div>
    )
}
...
function Droppable(props) {
    ...
    return (
        <div onDragOver={handleDragOver} onDrop={handleDrop} className={className}>{children}</div>
    )
}
...
function Ourwork() {
    ...
    function handleDrop(e, i) {
        if (typeof (index) === 'number' && typeof (i) === 'number') {
            ...
            setList([...list.slice(0, a), list[b], ...list.slice(a + 1, b), list[a], ...list.slice(b + 1)])
        }
    }
    ...
    return (
      ...
      <div className="container bkg">
                <div className="h1 text-center text-white text-shadow pt-10 titleSize">OUR <b>MAGICAL WORKS</b></div>
        <div className="p-3">
          <div className="form-row">
            {list.map((e,i)=>{
              return (
                  <div className="col-4">
                  <Droppable onDrop={handleDrop} onDragOver={handleDragOver} value={i}>
                          <Draggable onDragStart={handleDragStart} onDragEnd={handleDragEnd} value={i} >
                              <div className="myImg">
                                  <img key={i} src={e} className="w-100 border p-1 m-2 shadow-sm" style={{ maxHeight: 150, objectFit: 'contain' }} />
                              </div>
                    </Draggable>
                  </Droppable>
                </div>
          ...
          <div className="text-center mb-3 mt-6">
            <button className="btn btn-success btn-sm pl-3 pr-3 rounded-pill shadow" onClick={handleUpload}>Add More</button>
            <input className="d-none" type="file" ref={btn} onChange={handleChange} />
          </div>
        </div>
      </div>
    </FlexContainer>
